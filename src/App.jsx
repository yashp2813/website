...
import { 
  Calculator, Package, Building2, Users, History, LogOut, Plus, Trash2, Lock, ShieldAlert, CheckCircle2, Download, Upload, Factory, Coins, PieChart, ShoppingCart, Edit2, Archive, Search, Truck
} from 'lucide-react';
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'mahapack-erp';
...
const handleCSVImport = async (e, collectionName, getColRef, addLog) => {
  ...
  reader.onload = async (event) => {
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        let obj = {};
        headers.forEach((header, index) => { if (header) obj[header] = values[index] !== undefined ? values[index] : ''; });
        if (Object.keys(obj).length > 0) {
          await addDoc(getColRef(collectionName), obj);
          importedCount++;
        }
      }
...
export default function App() {
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      select, input, textarea { color: #1c1917 !important; background-color: #ffffff !important; color-scheme: light !important; }
      select option { color: #1c1917 !important; background-color: #ffffff !important; }
      select:disabled, input:disabled { background-color: #f5f5f4 !important; color: #78716c !important; }
    `;
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(getColRef('erp_users'), (snap) => setErpUsers(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubCompanies = onSnapshot(getColRef('companies'), (snap) => setCompanies(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubItems = onSnapshot(getColRef('items'), (snap) => setItems(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubProduction = onSnapshot(getColRef('production'), (snap) => setProduction(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubOrders = onSnapshot(getColRef('orders'), (snap) => setOrders(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubWastage = onSnapshot(getColRef('wastage'), (snap) => setWastageLogs(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubInventory = onSnapshot(getColRef('inventory'), (snap) => setInventory(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
    const unsubLogs = onSnapshot(getColRef('logs'), (snap) => setLogs(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
...
function FinishedGoodsView({ orders, production, items, companies, addLog, getDocRef, currentUser }) {
  ...
  const handleDispatch = async (e, order, inStock) => {
    e.preventDefault();
    const qty = parseInt(e.target.dispatchQty.value);
        ...
        <table className="w-full text-left min-w-[1200px]">
          ...
          <tbody className="divide-y divide-stone-200">
            {visibleOrders.map(order => {
              ...
              let effectiveBase = 0;
              if (ply <= 2) {
                effectiveBase = sumBoard + sumLiner; 
              } else {
              ...
              // Calculate Weights and Values using Net Weight (Gross UseKG minus Waste Sheets KG)
                  ...
                  <td className="p-4 text-right">
                    {inStock > 0 ? (
                      <form onSubmit={(e) => handleDispatch(e, order, inStock)} className="flex items-center justify-end gap-2">
                        <input required type="number" min="1" max={inStock} name="dispatchQty" className="w-24 p-2 border border-stone-300 rounded text-sm bg-white focus:ring-2 focus:ring-stone-800 focus:outline-none" placeholder="Qty..." />
...
function InventoryView({ inventory, production, addLog, role, getColRef, getDocRef }) {
  ...
  const [newReel, setNewReel] = useState({
    ...
    millName: '', invoiceNo: '', vehicleNo: '', reelNo: '', 
    size: '', gsm: '', bf: '', colour: 'Kraft', receivedQty: '', ratePerKg: ''
  });

  const [filters, setFilters] = useState({
    searchReel: '', size: '', gsm: '', bf: '', colour: '', status: 'All'
  });
  ...
  const handleDelete = async (id, reelNo) => {
    if(window.confirm(`Delete inventory record for Reel ${reelNo}?`)) {
      await deleteDoc(getDocRef('inventory', id));
      addLog(`Deleted inventory reel: ${reelNo}`);
    }
  };
  ...
  sortedProd.forEach(p => {
    ...
    pReels.forEach((rNo, index) => {
      ...
      if (deduct > 0) {
        ...
        usageStats[rNo].log.push({
          date: p.date,
          usedFor: p.usedForItem || p.paperUsedFor || 'Unknown',
          kg: deduct.toFixed(1)
        });
        remainingUse -= deduct;
      }
  ...
  const handleExport = () => {
    const exportData = filteredInventory.map(reel => ({
      Date: reel.date || '',
      Mill_Name: reel.millName || '',
      Invoice_No: reel.invoiceNo || '',
      ...
      Issued_Qty: reel.issuedQty.toFixed(2),
      Balance_Qty: reel.balanceQty.toFixed(2),
      Rate_per_KG: reel.ratePerKg,
      Current_Value: reel.value.toFixed(2),
    ...
    <div className="max-w-6xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <label className="flex items-center gap-2 bg-stone-200 text-stone-800 px-4 py-2 rounded-lg hover:bg-stone-300 font-medium text-sm transition cursor-pointer">
            ...
            <input type="file" accept=".csv" className="hidden" onChange={(e) => handleCSVImport(e, 'inventory', getColRef, addLog)} />
          </label>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-stone-200 mb-6">
        <h3 className="font-bold mb-4 flex items-center gap-2">
          ...
          {editingId ? 'Edit Reel Entry' : 'Receive New Reel'}
        </h3>
        <form onSubmit={handleAddOrUpdate} className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Date Received</label>
            <input required type="date" className="w-full p-2 border rounded" value={newReel.date} onChange={e => setNewReel({...newReel, date: e.target.value})} />
          </div>
          <div className="col-span-1 md:col-span-2">
            <label className="block text-xs text-stone-500 mb-1">Mill Name</label>
            <input required type="text" className="w-full p-2 border rounded" value={newReel.millName} onChange={e => setNewReel({...newReel, millName: e.target.value})} />
          </div>
          <div className="col-span-1 md:col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Invoice No.</label>
            <input type="text" className="w-full p-2 border rounded" value={newReel.invoiceNo} onChange={e => setNewReel({...newReel, invoiceNo: e.target.value})} />
          </div>
          <div className="col-span-1 md:col-span-2">
            <label className="block text-xs text-stone-500 mb-1">Vehicle No.</label>
            <input type="text" className="w-full p-2 border rounded" value={newReel.vehicleNo} onChange={e => setNewReel({...newReel, vehicleNo: e.target.value})} />
          </div>

          <div className="col-span-1 md:col-span-2">
            <label className="block text-xs font-bold text-blue-700 mb-1">Reel No. (Must be unique)</label>
            <input required type="text" className="w-full p-2 border border-blue-300 bg-blue-50 rounded font-mono font-bold text-stone-900" placeholder="e.g. 101" value={newReel.reelNo} onChange={e => setNewReel({...newReel, reelNo: e.target.value})} />
          </div>
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Size (mm/in)</label>
            <input required type="text" className="w-full p-2 border rounded" value={newReel.size} onChange={e => setNewReel({...newReel, size: e.target.value})} />
          </div>
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">GSM</label>
            <input required type="number" step="0.1" className="w-full p-2 border rounded" value={newReel.gsm} onChange={e => setNewReel({...newReel, gsm: e.target.value})} />
          </div>
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">BF</label>
            <input required type="number" step="0.1" className="w-full p-2 border rounded" value={newReel.bf} onChange={e => setNewReel({...newReel, bf: e.target.value})} />
          </div>
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Colour</label>
            <select required className="w-full p-2 border rounded" value={newReel.colour} onChange={e => setNewReel({...newReel, colour: e.target.value})}>
          ...
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Received Qty (KG)</label>
            <input required type="number" step="0.1" className="w-full p-2 border rounded bg-green-50" value={newReel.receivedQty} onChange={e => setNewReel({...newReel, receivedQty: e.target.value})} />
          </div>
          <div className="col-span-1">
            <label className="block text-xs text-stone-500 mb-1">Rate / KG (₹)</label>
            <input required type="number" step="0.01" className="w-full p-2 border rounded" value={newReel.ratePerKg} onChange={e => setNewReel({...newReel, ratePerKg: e.target.value})} />
          </div>

          <div className="col-span-1 lg:col-span-4 flex gap-2 mt-2">
            <button type="submit" className="flex-1 bg-stone-900 text-white p-2 rounded flex items-center justify-center gap-2 hover:bg-stone-800">
              {editingId ? <><Edit2 className="w-4 h-4" /> Update Reel</> : <><Plus className="w-4 h-4" /> Save to Inventory</>}
            </button>
            {editingId && (
              <button type="button" onClick={cancelEdit} className="bg-stone-300 text-stone-800 p-2 rounded hover:bg-stone-400 px-6">
                Cancel
              </button>
            )}
          </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-200 mb-6 flex flex-wrap gap-3 items-center">
        ...
        <select className="p-2 border rounded text-sm" value={filters.colour} onChange={e => setFilters({...filters, colour: e.target.value})}>
          <option value="">All Colours</option>
          <option value="Kraft">Kraft</option>
          <option value="Golden">Golden</option>
          <option value="White">White</option>
        </select>
        <select className="p-2 border rounded text-sm font-bold bg-stone-50" value={filters.status} onChange={e => setFilters({...filters, status: e.target.value})}>
          <option value="All">All Statuses</option>
          <option value="Available">Only Available (Balance &gt; 0)</option>
          <option value="Used">Used / Empty (Balance = 0)</option>
        </select>
        ...
        <table className="w-full text-left min-w-[1200px]">
          ...
          <tbody className="divide-y divide-stone-200 text-sm">
            {filteredInventory.length === 0 && (
              <tr><td colSpan="9" className="p-4 text-center text-stone-500">No inventory records found.</td></tr>
            )}
            {[...filteredInventory].sort((a,b) => new Date(b.date) - new Date(a.date)).map(reel => {
              ...
              <tr key={reel.id} className={`hover:bg-stone-50 ${!isAvailable ? 'opacity-60 bg-stone-50' : ''}`}>
                <td className="p-4">
                  <div className="font-medium">{reel.date}</div>
                  <div className="text-xs text-stone-500">{reel.millName}</div>
                  <div className="text-[10px] text-stone-400">Veh: {reel.vehicleNo || '-'}</div>
                </td>
                <td className="p-4">
                  <span className={`font-mono font-bold text-lg ${isAvailable ? 'text-blue-700' : 'text-stone-500'}`}>{reel.reelNo}</span>
                  {!isAvailable && <span className="ml-2 text-[10px] bg-stone-300 px-1 py-0.5 rounded text-stone-700 font-bold">EMPTY</span>}
                </td>
                <td className="p-4 text-stone-600">
                  <div>{reel.size}</div>
                  <div className="text-xs">{reel.gsm} GSM | {reel.bf} BF</div>
                  <div className="text-xs">{reel.colour}</div>
                </td>
                ...
                <td className="p-4 bg-green-50/30">
                  <span className={`font-bold text-lg ${isAvailable ? 'text-green-700' : 'text-stone-500'}`}>{reel.balanceQty.toFixed(1)} kg</span>
                </td>
                <td className="p-4">
                  <div className="text-xs text-stone-500 mb-1">Rate: ₹{reel.ratePerKg.toFixed(2)}/kg</div>
                  <div className="font-bold text-stone-800 text-base">₹{reel.value.toFixed(2)}</div>
                </td>
                <td className="p-4">
                  {reel.usageLog.length === 0 ? <span className="text-stone-400 italic text-xs">Not used yet</span> : (
                    <ul className="text-xs space-y-1">
                      {reel.usageLog.map((log, idx) => (
                        <li key={idx} className="flex gap-2">
                          <span className="text-stone-400">{log.date}</span>
                          <span className="font-medium text-stone-700">{log.usedFor}</span>
                          <span className="text-orange-600 font-mono">({log.kg}kg)</span>
                        </li>
                      ))}
                    </ul>
                  )}
                ...
                {role === 'admin' && (
                  <td className="p-4 text-right whitespace-nowrap">
                    <button onClick={() => handleEdit(reel)} className="text-blue-500 hover:text-blue-700 mr-3" title="Edit"><Edit2 className="w-5 h-5 inline" /></button>
                    <button onClick={() => handleDelete(reel.id, reel.reelNo)} className="text-red-500 hover:text-red-700" title="Delete"><Trash2 className="w-5 h-5 inline" /></button>
                  </td>
                )}
              </tr>
              );
    ...
    <div className="max-w-6xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <label className="flex items-center gap-2 bg-stone-200 text-stone-800 px-4 py-2 rounded-lg hover:bg-stone-300 font-medium text-sm transition cursor-pointer">
            <Upload className="w-4 h-4" /> Import CSV
            <input type="file" accept=".csv" className="hidden" onChange={(e) => handleCSVImport(e, 'production', getColRef, addLog)} />
          </label>
          <button onClick={handleExport} className="flex items-center gap-2 bg-stone-200 text-stone-800 px-4 py-2 rounded-lg hover:bg-stone-300 font-medium text-sm transition">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-stone-200 mb-8">
        ...
        <form onSubmit={handleAddOrUpdate} className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
          ...
          <div className="col-span-1 md:col-span-2"><label className="block text-xs text-stone-500 mb-1">Used For Item</label><select required className="w-full p-2 border rounded" value={newRecord.usedForItem} onChange={e => setNewRecord({...newRecord, usedForItem: e.target.value})} disabled={!!newRecord.orderId}><option value="">-- Select Item --</option>{[...visibleItems].filter(i => i.companyId === newRecord.companyId || !newRecord.companyId).sort((a,b) => (a?.name || '').localeCompare(b?.name || '')).map(i => <option key={i.id} value={i.name || i.Item_Name}>{i.name || i.Item_Name}</option>)}</select></div>
    ...
    <div className="max-w-6xl mx-auto pb-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <label className="flex items-center gap-2 bg-stone-200 text-stone-800 px-4 py-2 rounded-lg hover:bg-stone-300 font-medium text-sm transition cursor-pointer">
            <Upload className="w-4 h-4" /> Import CSV
            <input type="file" accept=".csv" className="hidden" onChange={(e) => handleCSVImport(e, 'items', getColRef, addLog)} />
          </label>
          <button onClick={handleExport} className="flex items-center gap-2 bg-stone-200 text-stone-800 px-4 py-2 rounded-lg hover:bg-stone-300 font-medium text-sm transition">
            <Download className="w-4 h-4" /> Export to Excel
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden overflow-x-auto">
        <table className="w-full text-left min-w-[800px]">
          ...
          <tbody className="divide-y divide-stone-200">
            {visibleItems.length === 0 && <tr><td colSpan="5" className="p-4 text-center text-stone-500">No items found. Add one above.</td></tr>}
